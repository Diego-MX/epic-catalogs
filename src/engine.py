...
def clabe_parse(clabe_key):
    """
    Obtención de la clabe del banco, con los siguientes datos:
        - name
        - code
        - banxicoId
        - alias
        - spei
        - portability
    La información proviene de df_banks
    """
    try:
        gfb_code = '072'
        bineo_code = '165'
...
def card_number_parse(card_num, response_obj='bin'):
    """
    Obtención del bin de la tarjeta, la información proviene de national_banks_bins
    """
    try:
        if len(card_num) != 16:
        ...
        bins_df = (pd.read_sql_table('national_banks_bins',engine_card)
            .set_index('BIN')
...
def bank_acquiring(acquiring_code):
    """Extracción de datos para, la tabala utilizada es:
            - national_banks_acquiring
    """
    try:
        acq_cols = {
            'Institución' : 'name', 
        ...
        acquiring_banks = (pd.read_sql_table("national_banks_acquiring",engine_acquiring)
...
def zipcode_query(a_zipcode):
    """Extracción de datos para el armado del zipcode, las tabalas utilizadas son:
            - codigos_drive_tipo_asentamientos
            - codigos_drive_ciudades
            - codigos_drive_municipios
            - estados_claves
            - codigos_drive 
    """
    try:
        engine_zipcode = tools.get_connection()
        tipo_asenta = pd.read_sql_table('codigos_drive_tipo_asentamientos',engine_zipcode)
        ciudades = pd.read_sql_table('codigos_drive_ciudades',engine_zipcode)
        municipios =(pd.read_sql_table('codigos_drive_municipios',engine_zipcode)
                .assign(cve_mnpio=lambda df: df.c_estado.str.cat(df.c_mnpio)))

        estados = (pd.read_sql_table('estados_claves',engine_zipcode)
        ...
        las_colonias = (pd.read_sql_table('codigos_drive',engine_zipcode)
